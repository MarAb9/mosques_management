...
check('Responsible values mapped', $responsibles[0]['responsible_name'] === 'مسؤول تجريبي'
    ...
    && $responsibles[0]['challenges'] === 'تحدي <تجريبي>');
...
check('Edit form escapes responsible free text', str_contains($r['body'], 'تحدي &lt;تجريبي&gt;')
    && !str_contains($r['body'], 'تحدي <تجريبي>'));
preg_match('/name="csrf_token" value="([a-f0-9]{64})"/', $r['body'], $m);
$token = $m[1] ?? $token;
...
// ── POST delete (bulk selected) ───────────────────────────────────────────────
$freeCodes = $pdo->query('
    SELECT m.national_code FROM mosques m
    LEFT JOIN quran_memorization_programs q ON m.national_code = q.mosque_registration_number
    WHERE q.id IS NULL LIMIT 2
')->fetchAll(PDO::FETCH_COLUMN);
check('Bulk delete setup has two free mosques', count($freeCodes) === 2);

$bulkIds = [];
$insertProgram = $pdo->prepare('INSERT INTO quran_memorization_programs (mosque_registration_number, has_quran_school, has_accommodation) VALUES (?, ?, ?)');
$insertResponsible = $pdo->prepare('INSERT INTO quran_program_responsibles (program_id, responsible_name, memorization_schedule) VALUES (?, ?, ?)');
foreach ($freeCodes as $index => $code) {
    $insertProgram->execute([$code, 'نعم', 'لا']);
    $bulkId = (int) $pdo->lastInsertId();
    $bulkIds[] = $bulkId;
    $insertResponsible->execute([$bulkId, 'مسؤول حذف جماعي ' . ($index + 1), 'باستمرار']);
}

$r = req('delete_quran_mosque.php', [CURLOPT_POSTFIELDS => http_build_query([
    'csrf_token' => $token,
    'selected_mosques' => $bulkIds,
])]);
check('Bulk delete redirects', $r['status'] === 302 && str_contains($r['redirect'], 'quran_mosques.php'));

$bulkIdList = implode(',', array_map('intval', $bulkIds));
$bulkCount = (int) $pdo->query("SELECT COUNT(*) FROM quran_memorization_programs WHERE id IN ({$bulkIdList})")->fetchColumn();
$bulkOrphans = (int) $pdo->query("SELECT COUNT(*) FROM quran_program_responsibles WHERE program_id IN ({$bulkIdList})")->fetchColumn();
check('Bulk programs deleted with responsibles', $bulkCount === 0 && $bulkOrphans === 0);
