...
// Fetch mosque data
$stmt = $pdo->prepare("SELECT * FROM mosques WHERE registration_number = ?");
$stmt->execute([$id]);
$mosque = $stmt->fetch();
...
// Convert construction_date to year for display
$formData = $mosque;
$formData['construction_year'] = !empty($mosque['construction_date']) 
    ? date('Y', strtotime($mosque['construction_date'])) 
    : '';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        if (empty($errors)) {
            ...
            if (!empty($_FILES['main_image']['name'])) {
                ...
                if (!empty($uploadErrors)) {
                    ...
                    if (move_uploaded_file($file['tmp_name'], $destination)) {
                        ...
                        if (!empty($formData['main_image']) && file_exists($formData['main_image'])) {
                            unlink($formData['main_image']);
                        }
                        $formData['main_image'] = $destination;
                    } else {
            ...
            if (empty($errors)) {
                $stmt = $pdo->prepare("UPDATE mosques SET 
                    mosque_name = ?,
                    address = ?,
                    construction_date = ?,
                    national_code = ?,
                ...
                $_SESSION['success'] = "تم تحديث بيانات المسجد بنجاح";
                header("Location: mosques.php");
                exit();
            }
        }
