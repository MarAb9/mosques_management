...
if (!$program) {
    $_SESSION['error'] = 'مسجد التحفيظ غير موجود';
    header('Location: quran_mosques.php');
    exit;
}

// Fetch responsibles
$responsiblesStmt = $pdo->prepare("SELECT * FROM quran_program_responsibles WHERE program_id = ?");
...
$scheduleOptions = $enumValues['memorization_schedule'] ?? [];

// Get position options
$positionOptions = ['مدرر(ة)','مرشد(ة)','إمام','متطوع(ة)','رسمي(ة)'];

// Get list of mosques with national_code
$mosques = $pdo->query("
    SELECT 
        national_code AS id, 
        mosque_name,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $updateData = [
            'mosque_registration_number' => (string)filter_input(INPUT_POST, 'mosque_registration_number', FILTER_SANITIZE_FULL_SPECIAL_CHARS),
            'has_quran_school' => $_POST['has_quran_school'] ?? 'لا',
            'has_accommodation' => $_POST['has_accommodation'] ?? 'لا',
            'id' => $programId
        ];
        ...
        $deleteStmt = $pdo->prepare("DELETE FROM quran_program_responsibles WHERE program_id = ?");
        $deleteStmt->execute([$programId]);
        
        // Insert updated responsibles
        if (!empty($_POST['responsibles'])) {
            ...
            foreach ($_POST['responsibles'] as $responsible) {
                $responsibleStmt->execute([
                    $programId,
                    filter_var($responsible['name'], FILTER_SANITIZE_FULL_SPECIAL_CHARS),
                    filter_var($responsible['position'] ?? '', FILTER_SANITIZE_FULL_SPECIAL_CHARS),
                    filter_var($responsible['national_id'] ?? '', FILTER_SANITIZE_FULL_SPECIAL_CHARS),
                    $responsible['has_work_program'] ?? 'لا',
                    filter_var($responsible['memorization_schedule'] ?? '', FILTER_SANITIZE_FULL_SPECIAL_CHARS),
                    ...
                    filter_var($responsible['challenges'] ?? '', FILTER_SANITIZE_FULL_SPECIAL_CHARS),
                    filter_var($responsible['notes_suggestions'] ?? '', FILTER_SANITIZE_FULL_SPECIAL_CHARS)
                ]);
            }
        }
        ...
        $_SESSION['success'] = 'تم تحديث مسجد التحفيظ بنجاح';
        header('Location: quran_mosques.php');
        exit;
